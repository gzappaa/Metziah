...
# Extracts the time component from PromoFull filenames.
# Used to determine which same-day PromoFull file is newest when
# no separate date_key/date_format metadata field is available.
...
def find_latest_promofull_files_per_store(
    files: list[dict],
    filename_key: str,
    ...
    """
    Find the latest PromoFull file per chain, store and day.

    For sources where multiple PromoFull files exist for the same
...
def _cleanup_old_promofull_files(
    folder: Path,
    keep_filename: str,
    file_date: date,
) -> None:
    """
    Remove older same-day PromoFull files for this store, now that
...
async def save_promofull_file_async(
    chain_id: str,
    store_id: str,
    filename: str,
    file_date: date,
...
def list_publishedprices_entries_recursive(
    ...
    since PromoFull, like PriceFull, may have several live files at
    once across different stores/folders.
    """
...
def download_promofull_publishedprices(
    ...
    latest_files = find_latest_promofull_files_per_store(
        entries,
        filename_key="fname",
    )
    ...
    if not latest_files:
        logger.warning(
            "No PromoFull files found for %s",
            name,
        )
        return []
...
async def download_promofull_laibcatalog(
    ...
    for latest in latest_files.values():
        ...
        result = await save_promofull_file_async(
...
async def download_promofull_carrefour(
    test: bool = False,
) -> list[Path]:
    ...
    if not latest_files:
        logger.warning(
            "No PromoFull files found for Carrefour"
        )
        return []
...
async def get_all_html_candidates(client, listing_config):
        ...
        results = await asyncio.gather(
            *(
                get_html_page(
                    ...
                    current_page,
                    page_param,
                    listing_config.get("promofull_params"),
                )
...
async def download_promofull_html(
    ...
    # "PriceFull" is configured in pricesfull.py. Verify against live
    # config — some sources may not publish a distinct PromoFull filter.
    promofull_params = categories.get(
        "file_types",
        {},
    ).get("PromoFull")
    ...
    if promofull_params is None:
        logger.warning(
            "No 'PromoFull' file_type configured for %s",
            name,
        )
        return []
    ...
    try:
        candidates = await get_all_html_candidates(
            ...
            {
                **listing,
                "promofull_params": promofull_params,
            },
        )
...
async def download_promofull_mishnatyosef(
    ...
    if not latest_files:
        logger.warning(
            "No PromoFull files found for Mishnat Yosef"
        )
        return []
...
async def download_promofull_wolt(
    test: bool = False,
) -> list[Path]:
    ...
    if not latest_files:
        logger.warning(
            "No PromoFull files found for Wolt"
        )
        return []
...
if __name__ == "__main__":
    ...
    logger.info(
        "Finished. Downloaded %d PromoFull file(s) total.",
        len(all_downloaded),
    )