...
def download_pricefull_publishedprices(
    ...
    latest_files = trim_for_test(latest_files, name) if test else latest_files
...
def download_pricefull_binaprojects(
    ...
    try:
        ...
        files = client.get_hok_files(file_type=4)
    except Exception:
...
async def download_pricefull_carrefour(test: bool = False) -> list:
...
async def download_pricefull_html(source: dict, test: bool = False) -> list:
...
async def download_pricefull_mishnatyosef(test: bool = False) -> list:
